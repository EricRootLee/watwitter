defmodule Watwitter.Timeline do
  ...
  def like_post!(%Post{} = post, %User{} = user) do
    ...
    broadcast_post_update(updated_post)
    updated_post
  end
  ...
  @topic "timeline"
  def subscribe do
    Phoenix.PubSub.subscribe(Watwitter.PubSub, @topic)
  end

  def broadcast_post_creation(post) do
    Phoenix.PubSub.broadcast(Watwitter.PubSub, @topic, {:post_created, post})
  end

  def broadcast_post_update(post) do
    Phoenix.PubSub.broadcast(Watwitter.PubSub, @topic, {:post_updated, post})
  end

  defp broadcast({:error, _} = error, _), do: error

  defp broadcast({:ok, post} = ok_tuple, :post_created) do
    broadcast_post_creation(post)
    ok_tuple
  end
end
